...
function exec(service, action, args) {
    ...
    if (DEVICE_IS_IOS) {
        ...
        if (status == 0) {
            ...
            //alert("service:" + service + " action:" + action + " error:" + message);
        }
    }
...
BestPay_Payment.prototype = {
    ...
    /**
     * 取得默认资金源
     * @method getMoneySource
     * @param {[Json]} params 支付参数
     *                        {
     *                        MERCHANTID:0, //商户号
     *                        ORDERSEQ:0, //订单号
     *                        ORDERREQTRANSEQ:0, //订单流水号
     *                        ORDERAMOUNT:0, //订单金额
     *                        BUSITYPE:0 //交易类型
     *                        }
     * @param {[Function]} callback 支付成功回调
     ...
     * @example
     * var params={
     *           MERCHANTID:0, //商户号,默认0
     *           ORDERSEQ:0, //订单号,默认0
     *           ORDERREQTRANSEQ:0, //订单流水号,默认0
     *           ORDERAMOUNT:0, //订单金额,默认0
     *           BUSITYPE:0 //交易类型,默认0
     *           }
     *
     ...
     *      alert("获取失败");
     *  }
     *  Payment.getMoneySource(params, success, fail);
     */
    getMoneySource: function(params, callback, fail) {
        var json = {
            'params': params
        };
        return exec_asyn("Payment", "getMoneySource", json, callback, fail);
    },
    /**
     * 更换资金源
     * @method changeMoneySource
     ...
     *      alert("获取失败");
     *  }
     *  Payment.changeMoneySource(params, success, fail);
     */
    changeMoneySource: function(params, callback, fail) {
        var json = {
            'params': params
        };
        return exec_asyn("Payment", "changeMoneySource", json, callback, fail);
    }
}
...
BestPay_LBS.prototype = {
    ...
    getInfo: function(params, callback, fail) {
        ...
        return exec_asyn("LBS", "getInfo", params, callback, fail);
    }
...
BestPay_Scanner.prototype = {
     ...
     * @method scan
     * @param {[Json]} params 自定义参数
     *                            -type:识别类型 值为qrCode 二维码、 barCode 条码、 bankCard 银行卡、barCodeAndQrCode 条码和二维码
     * @param {[Function]} callback － 成功返回时执行的函数
     * @param {[Function]} fail － 失败返回时执行的函数
     * @return {[String||Object]} 扫码后得到的字符串或对象（银行卡）
     *                --银行卡对象 {cardType:卡种类 如信用卡或借记卡,
     *                                cardNo:卡号,
     *                                cardName:银行卡名称}
     * @example
...
BestPay_User.prototype = {
    ...
    getClientInfo: function() {
        return exec("User", "getClientInfo", {});
    }
};
...
BestPay_App.prototype = {
    ...
    /**
     * 隐藏topbar右上角内容         android: >=5.0.4, ios: >=5.0.4
    ...
    /**
     * 隐藏topbar右上角图片，不推荐使用，使用App.showTopRightBar 代替
     ...
     *                              MSGID: ""   //公告ID
     *                          }
     *      11:我的代金券 version>=5.1.4 (2016.4.18)
     *      12:优惠活动 version>=5.1.4
     */
    ...
    * @param {[Boolean]} isVisible 是否显示
    * @example
    *  App.setCloseButtonVisible(false);
    */
    setCloseButtonVisible: function(isVisible) {
        exec("App", "setCloseButtonVisible", { isVisible: isVisible });
     },
     /**
     ...
     * @param {[String]} colorValue 手机状态栏颜色
     * @example
     * App.setStatusBarColor('#1c1c1c');
     *传值为normal时，是手机状态栏的颜色改成默认的(暂时没有加，加的时候，给干掉)
     * App.setStatusBarColor('normal');
     */
    setStatusBarColor: function(colorValue) {
        var json ={
            'colorValue':colorValue
        }
        exec("App", "setStatusBarColor",json);
    }
};
...
BestPay_Util.prototype = {
     ...
     *  var result =util.base64Encode(source);
     ...
     *  var result = util.base64Decode(source);
...
BestPay_Dialog.prototype = {
     ...
     * @param  {[defalutDate:"yyyy-MM-dd"]} defalutDate表示默认日期
     * @example
     *  Dialog.showDatePicker({defalutDate:"2017-02-09"},sucessCallback,failCallback,cacelCallback);
     */
    showDatePicker: function(param,sucessCallback,failCallback,cacelCallback) {

        return exec_asyn("Dialog", "showDatePicker", param,sucessCallback,failCallback,cacelCallback);
    },
...
BestPay_Security.prototype = {
    ...
    /**
     * 开放平台内容加密 6.0.1以上提供
     * @method methodApiEncrypt
     * @return 全报文加密，供开放平台使用
     * @example
     *  Security.methodApiEncrypt({"a":1}function(result){
     *     alert(result);
     *  }, function(){
     *      alert("fail");
     *  });
     *
     */
    methodApiEncrypt: function(args,success, fail) {
        exec_asyn("Security", "methodApiEncrypt", args, success, fail);
    }
...
BestPay_Camera.prototype.saveBase64Image = function(args, success, fail) {
    /**
     * 保存图片到相册      > 5.1.6
     * @method saveBase64Image
     * @param {[Json]} json bundle参数
     * @param {[Function]} success 调用成功返回时调用的函数
     * @param {[Function]} fail 调用失败时执行的函数
     * @example
     *  bp.saveBase64Image({
     *      base64ImageStream: stream
     *  },success,error);
     *
     */
    return exec_asyn("Camera", "saveBase64Image", args, success, fail);
}

BestPay_Camera.prototype.chooseImage = function(args, success, fail, cancel) {
    /**
     * 拍照或从相册中选择照片      >= 5.0.5
     * @method chooseImage
     ...
     *      maxWidth: 500,  //j图片的最大宽度. 过大将被等比缩小,为0则不缩放
     *      maxHeight: 500, //图片的最大高度. 过大将被等比缩小，为0则不缩放
     *      enableAlbum:true,//允许使用相册 默认true
     ...
     *  }, function(failData){    //成功选择图片后的回调
     *      alert(failData);
     *  }, function(cancelData){    //成功选择图片后的回调
     *      alert(cancelData);
     *  });
     */
    exec_asyn("Camera", "chooseImage", args, success, fail,cancel);
}
